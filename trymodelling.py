...
churnspace=churnspace.drop(churnspace.columns[5:15],axis=1)

#churnspace = churnspace.drop([4:10],axis=1)
...
counts.to_csv("predictionresult.csv",sep=',');
...
churnspace=pd.read_csv(sys.argv[2])
...
print churnspace['customerID']

r=churnspace

x=churnspace[features].as_matrix().astype(float)
...
r["churn_prob"]=churn_prob[:,1]

r=r.drop(r.columns[1:20],axis=1)

r["customer_worth"]=churnspace["TotalCharges"]
...
os.system("cp FinalResult.csv public/FinalResult.csv")
